  ...
  )
{
  ...
  if (ConfigData->EnableDnsCache) {
    NET_LIST_FOR_EACH_SAFE (Entry, Next, &mDriverData->Dns4CacheList) {
      Item = NET_LIST_USER_STRUCT (Entry, DNS4_CACHE, AllCacheLink);
      if (CompareMem (&IpAddress, Item->DnsCache.IpAddress, sizeof (EFI_IPv4_ADDRESS)) == 0) {
        Token->RspData.A2HData = AllocatePool (sizeof (DNS_ADDR_TO_HOST_DATA));
        if (Token->RspData.A2HData == NULL) {
          Status = EFI_OUT_OF_RESOURCES;
          goto ON_EXIT;
        }

        Token->RspData.A2HData->HostName = AllocateZeroPool (2 * StrLen (Item->DnsCache.HostName) + 2);
        if (Token->RspData.A2HData->HostName == NULL) {
          Status = EFI_OUT_OF_RESOURCES;
          goto ON_EXIT;
        }

        StrCpy (Token->RspData.A2HData->HostName, Item->DnsCache.HostName);
  ...
  TokenEntry->PacketToLive = Token->RetryInterval;
  CopyMem (&(TokenEntry->QueryIpAddress), &IpAddress, sizeof (EFI_IPv4_ADDRESS));
  TokenEntry->Token = Token;

  //
  // Construct DNS Query Packet.
  //
  CopyMem (&Ip.v4, &(TokenEntry->QueryIpAddress), sizeof (EFI_IPv4_ADDRESS));
  Status = ConstructDNSQueryHostName (Instance, Ip, DNS4_DOMAIN, &Packet);
  if (EFI_ERROR (Status)) {
    if (TokenEntry != NULL) {
      FreePool (TokenEntry);
    }
  ...
  )
{
  ...
  if (ConfigData->EnableDnsCache) {
    NET_LIST_FOR_EACH_SAFE (Entry, Next, &mDriverData->Dns6CacheList) {
      Item = NET_LIST_USER_STRUCT (Entry, DNS6_CACHE, AllCacheLink);
      if (CompareMem (&IpAddress, Item->DnsCache.IpAddress, sizeof (EFI_IPv6_ADDRESS)) == 0) {
        Token->RspData.A2HData = AllocatePool (sizeof (DNS6_ADDR_TO_HOST_DATA));
        if (Token->RspData.A2HData == NULL) {
          Status = EFI_OUT_OF_RESOURCES;
          goto ON_EXIT;
        }
  ...
  TokenEntry->PacketToLive = Token->RetryInterval;
  CopyMem (&(TokenEntry->QueryIpAddress), &IpAddress, sizeof (EFI_IPv6_ADDRESS));
  TokenEntry->Token = Token;

  //
  // Construct DNS Query Packet.
  //
  CopyMem (&Ip.v6, &(TokenEntry->QueryIpAddress), sizeof (EFI_IPv6_ADDRESS));
  Status = ConstructDNSQueryHostName (Instance, Ip, DNS6_DOMAIN, &Packet);
  if (EFI_ERROR (Status)) {
    if (TokenEntry != NULL) {
      FreePool (TokenEntry);
    }
