  ...
  )
{
  ...
  *Completed = TRUE;
  ...
  } else if (QuerySection->Type == DNS_TYPE_PTR) {
    if (Instance->Service->IpVersion == IP_VERSION_4) {
      Dns4TokenEntry->Token->RspData.A2HData = AllocatePool (sizeof (DNS_ADDR_TO_HOST_DATA));
    } else {
      Dns6TokenEntry->Token->RspData.A2HData = AllocatePool (sizeof (DNS6_ADDR_TO_HOST_DATA));
    }
  } else {
    ...
    if (AnswerSection->Type == QuerySection->Type) {
      switch (AnswerSection->Type) {
        ...
        case DNS_TYPE_PTR:
          ...
          if (Instance->Service->IpVersion == IP_VERSION_4) {
            Dns4TokenEntry->Token->RspData.A2HData->HostName = AllocateZeroPool (2 * (AnswerSection->DataLength - 1));
            HostName                                         = Dns4TokenEntry->Token->RspData.A2HData->HostName;
          } else {
            Dns6TokenEntry->Token->RspData.A2HData->HostName = AllocateZeroPool (2 * (AnswerSection->DataLength - 1));
            HostName                                         = Dns6TokenEntry->Token->RspData.A2HData->HostName;
          }
          ...
          while (*(UINT8 *)AnswerData != 0) {
            for (Index = 0; Index < *(UINT8 *)AnswerData; Index++) {
              HostName[Index] = (CHAR16)*(AnswerData + 1 + Index);
            }

            HostName   += (*(UINT8 *)AnswerData);
            *HostName++ = L'.';
            AnswerData += ((*(UINT8 *)AnswerData) + 1);
          }
          ...
          *(--HostName) = L'\0';
          ...
          if (Instance->Service->IpVersion == IP_VERSION_4) {
            ...
            Dns4CacheEntry            = AllocateZeroPool (sizeof (EFI_DNS4_CACHE_ENTRY));
            Dns4CacheEntry->IpAddress = AllocateZeroPool (sizeof (EFI_IPv4_ADDRESS));
            CopyMem (Dns4CacheEntry->IpAddress, &(Dns4TokenEntry->QueryIpAddress), sizeof (EFI_IPv4_ADDRESS));
            Dns4CacheEntry->HostName = AllocateZeroPool (2 * (StrLen (Dns4TokenEntry->Token->RspData.A2HData->HostName) + 1));
            CopyMem (Dns4CacheEntry->HostName, Dns4TokenEntry->Token->RspData.A2HData->HostName, 2 * (StrLen (Dns4TokenEntry->Token->RspData.A2HData->HostName) + 1));
            Dns4CacheEntry->Timeout = AnswerSection->Ttl;
            ...
            Dns6CacheEntry            = AllocateZeroPool (sizeof (EFI_DNS6_CACHE_ENTRY));
            Dns6CacheEntry->IpAddress = AllocateZeroPool (sizeof (EFI_IPv6_ADDRESS));
            CopyMem (Dns6CacheEntry->IpAddress, &(Dns6TokenEntry->QueryIpAddress), sizeof (EFI_IPv6_ADDRESS));
            Dns6CacheEntry->HostName = AllocateZeroPool (2 * (StrLen (Dns6TokenEntry->Token->RspData.A2HData->HostName) + 1));
            CopyMem (Dns6CacheEntry->HostName, Dns6TokenEntry->Token->RspData.A2HData->HostName, 2 * (StrLen (Dns6TokenEntry->Token->RspData.A2HData->HostName) + 1));
            Dns6CacheEntry->Timeout = AnswerSection->Ttl;
...
/**
  Construct the Packet to query HostName.

  @param  Instance              The DNS instance
  @param  IpAddress             Queried IpAddress
  @param  Domain                DNS query Domain
  @param  Packet                The packet for querying HostName
  ...
  )
{
  ...
  if (Instance->Service->IpVersion == IP_VERSION_4) {
    BufferSize = 2 * (sizeof ("255.255.255.255") + StrLen (Domain) + 1);
    ...
    ((UINT8 *)Buffer)[BufferSize - 1] = L'\0';
  } else {
    BufferSize = 2 * (sizeof "f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f.f" + StrLen (Domain) + 1);
    Buffer     = AllocateZeroPool (BufferSize);
    for (Index = 0, BufferCount = 0; Index <= 15; Index++) {
      Buffer[BufferCount] = (CHAR16)(((IpAddress.v6.Addr[15 - Index] & 0xF) < 9) ? \
                                     ((IpAddress.v6.Addr[15 - Index] & 0xF) + 0x30) : \
                                     ((IpAddress.v6.Addr[15 - Index] & 0xF) + 0x37));
      BufferCount++;
      Buffer[BufferCount] = L'.';
      BufferCount++;
      Buffer[BufferCount] = (CHAR16)((((IpAddress.v6.Addr[15 - Index] >> 4) & 0xF) < 9) ? \
                                     (((IpAddress.v6.Addr[15 - Index] >> 4) & 0xF) + 0x30) : \
                                     (((IpAddress.v6.Addr[15 - Index] >> 4) & 0xF) + 0x37));
      BufferCount++;
      Buffer[BufferCount] = L'.';
      BufferCount++;
    }

    CopyMem (Buffer + BufferCount, Domain, 2 * StrLen (Domain));
    ((UINT8 *)Buffer)[BufferSize - 1] = L'\0';
  }
  ...
  *Header  = (CHAR8)Len;
  *Tail    = 0;
  Frag.Len = (UINT32)(Frag.Len + StrLen (Buffer) + 2); /// 1 for header, 1 for tail.

  //
  // Rest query section
  //
  QuerySection        = (DNS_QUERY_SECTION *)(Frag.Bulk + Frag.Len);
  QuerySection->Type  = DNS_TYPE_PTR;
  QuerySection->Class = DNS_CLASS_INET;
